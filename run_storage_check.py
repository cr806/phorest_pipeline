...
logger = configure_logger(name=__name__, rotate_daily=False, log_filename='storage_check.log', log_to_terminal=True)
...
def check_usb_mount_and_permissions(mount_point: Path, test_file_name: Path, expected_uuid: str) -> bool:
    """
    ...
    """
    logger.info(f"Starting USB health check for mount point: {mount_point} with UUID: {expected_uuid}")
    ...
    try:
        ...
        if not usb_detected:
            ...
            if actual_mountpoint is None:
                logger.info(f"USB drive with UUID '{expected_uuid}' is detected but not currently mounted according to lsblk.")
    ...
    try:
        result = subprocess.run(['findmnt', '-n', '-o', 'TARGET', mount_point.as_posix()], 
                                capture_output=True, text=True, check=False)
        
        if result.returncode != 0 or result.stdout.strip() != mount_point.as_posix():
            logger.error(f"No filesystem mounted at '{mount_point}' or mount point mismatch (findmnt check failed).")
            if usb_detected and actual_mountpoint != mount_point:
                 logger.warning(f"USB detected, but mounted elsewhere ({actual_mountpoint}) or not at all, instead of '{mount_point}'.")
            return False
        
        logger.info(f"Filesystem confirmed to be mounted at: {mount_point} (via findmnt).")
    except FileNotFoundError:
        logger.error("[ERROR] 'findmnt' command not found. Please ensure 'util-linux' is installed.")
        return False
    except Exception as e:
    ...
    try:
        ...
        logger.info(f"USB drive at '{mount_point}' is mounted and has correct read/write permissions.")
        return True
    except PermissionError:
        logger.error(f"[ERROR] Permission denied: Cannot write to or delete from '{mount_point}'. "
                      f"        Check ownership and permissions.")
        return False
    except OSError as e:
        logger.error(f"[ERROR] Operating System error while testing permissions on '{mount_point}': {e}")
        return False
    except Exception as e:
