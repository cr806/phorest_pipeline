...
def perform_collection(
    ...
    match current_state:
        ...
        case CollectorState.COLLECTING:
            ...
            if ENABLE_CAMERA:
                ...
                try:
                    ...
                    if cam_status == 0:
                        ...
                        logger.info(f"Camera data collected: {cam_metadata_for_entry.get('filename')}")
                    else:
                        # Explicitly create error metadata
                        cam_metadata_for_entry = {
                            'error_flag': True,
                            'error_message': cam_msg,
                            'filename': cam_data_from_controller.get('filename') if cam_data_from_controller else None,
                            'timestamp_iso': cam_data_from_controller.get('timestamp_iso') if cam_data_from_controller else datetime.datetime.now().isoformat()
                        }
            ...
            if ENABLE_THERMOCOUPLE:
                ...
                try:
                    ...
                    if tc_status == 0:
                        ...
                        logger.info(f'Thermocouple data collected')
                    else:
                        temps_metadata_for_entry = {
                            'error_flag': True,
                            'error_message': tc_msg,
                            'data': tc_data_from_controller.get('data') if tc_data_from_controller else None,
                            'timestamp_iso': tc_data_from_controller.get('timestamp_iso') if tc_data_from_controller else datetime.datetime.now().isoformat()
                        }
            ...
            if ENABLE_CAMERA and (cam_metadata_for_entry is None or cam_metadata_for_entry.get('error_flag', True)):
                current_collection_successful = False
            if ENABLE_THERMOCOUPLE and (temps_metadata_for_entry is None or temps_metadata_for_entry.get('error_flag', True)):
                current_collection_successful = False
            ...
            try:
                ...
                logger.critical(f"Failed to add entry to processing manifest: {e}. This indicates a serious issue with file locking/writing.", exc_info=True)
                current_collection_successful = False
            ...
            if current_collection_successful:
                ...
                try:
                    ...
                    logger.critical(f"Could not create flag {DATA_READY_FLAG}: {e}. This will prevent processor from starting. Transitioning to FATAL_ERROR.", exc_info=True)
                    time.sleep(POLL_INTERVAL)
                    next_state = CollectorState.FATAL_ERROR
            else:
                logger.warning("Data collection cycle finished with errors or manifest write failed.")
