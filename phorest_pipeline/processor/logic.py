    ...
    def _perform_processing(self) -> None:
        ...
        match self.current_state:
            case ProcessorState.IDLE:
                ...
                logger.info(f"Next run time set to {self.next_run_time} (in {PROCESSOR_INTERVAL} seconds).")
            ...
            case ProcessorState.PROCESSING:
                ...
                if self.shutdown_requested:
                    self.current_state = ProcessorState.FATAL_ERROR  # Use FATAL_ERRROR to signal a clean stop
                else:
                    self.current_state = ProcessorState.PROCESSING  # Loop back immediately to check for more data
