...
def load_w1_modules():
    ...
    try:
        ...
        print("Error loading 1-Wire modules. Make sure you have sudo privileges and that 1-Wire is enabled.")
...
def find_thermocouple_serial_numbers():
    ...
    if not DEVICE_LOC.is_dir():
        print(f"[ERROR]: 1-Wire device directory not found at {DEVICE_LOC}. Is 1-Wire enabled and modules loaded?")
        return []
    ...
    for device_path in DEVICE_LOC.iterdir():
        ...
        if device_path.is_dir() and device_path.name.startswith('28-'):
            ...
            if Path(device_path, 'w1_slave').is_file():
                ...
                print(f"  Found directory '{serial_number}', but 'w1_slave' file is missing or unreadable.")
...
def main():
    ...
    if serial_numbers:
        print(f"{COLOR_GREEN}Successfully found {len(serial_numbers)} 1-Wire temperature sensor(s):{COLOR_END}")
        ...
        print(f"{COLOR_RED}No 1-Wire temperature sensors (devices starting with '28-') were found.{COLOR_END}")
        print("Possible reasons:")
        print("  - Sensors not physically connected to the 1-Wire bus.")
        print("  - 1-Wire not properly configured or enabled on your system (e.g., in /boot/config.txt for Raspberry Pi).")
        print("  - Required kernel modules ('w1-gpio', 'w1-therm') failed to load (check messages above).")
        print(f"{COLOR_BLUE}--------------------------------------------------{COLOR_END}")
