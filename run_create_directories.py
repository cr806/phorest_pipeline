...
def create_directories(usb_path: Path, directories: list[str]) -> bool:
    ...
    for directory_name in directories:
        ...
        try:
            ...
            logger.error(
                f"An unexpected error occurred while creating directory '{full_path}': {e}"
            )
            all_successful = False
...
def create_symlinks_to_usb(usb_path: Path, project_path: Path, directories: list[str]) -> bool:
    ...
    if not project_path.is_dir():
        logger.error(
            f"Project root path '{project_path}' does not exist or is not a directory. Cannot create symlinks."
        )
        return False
    ...
    for directory_name in directories:
        ...
        if not source_path.is_dir():
            logger.warning(
                f"Source directory '{source_path}' does not exist. Skipping symlink creation for this directory."
            )
            all_successful = False
            continue
        ...
        try:
            if link_path.is_symlink():
                if link_path.resolve() == source_path.resolve():
                    logger.info(
                        f'Symlink already exists and is correct: {link_path} -> {source_path}'
                    )
                else:
                    logger.warning(
                        f"Existing symlink '{link_path}' points to '{link_path.resolve()}' instead of '{source_path}'. Removing and recreating."
                    )
                    link_path.unlink()
                ...
                logger.error(
                    f"Cannot create symlink: A file or directory already exists at '{link_path}' and is not a symlink. Please move or delete it manually."
                )
                all_successful = False
            else:
                os.symlink(source_path, link_path)
            ...
            logger.error(
                f"An unexpected error occurred while creating symlink for '{directory_name}': {e}"
            )
            all_successful = False
...
if __name__ == '__main__':
    ...
    if not USB_MOUNT_POINT.is_dir():
        logger.critical(
            f"ERROR: USB mount point '{USB_MOUNT_POINT}' does not exist or is not a directory. Is the USB drive mounted correctly?"
        )
        logger.info(
            "Please ensure the USB drive is mounted at '/mnt/ARGUS_data' before running this script."
        )
        exit(1)
    ...
    if not PROJECT_ROOT.is_dir():
        logger.critical(
            f"ERROR: Project root directory '{PROJECT_ROOT}' does not exist or is not a directory. Please create it first."
        )
        exit(1)
